extends Sprite

export var speed_scale = 1.0
export var light_scale = 1.0
export var light_fade_scale = 0.9
export var can_shoot = true
export var bullets_per_burst_mod = 0
export var bullet_scale = 1.0
export var can_shoot_laser = true 
export var laser_scale = 1.0
export var can_bomb = true
export var bomb_scale = 1.0
export var max_bomb_mod = 0
export var points_scale = 1.0
export var laser_recharge_scale = 1.0
var player = null
export var bomb_scene_path = "res://Scenes/HelperScenes/Powerups/Bomb.tscn"
